...
import { ArrowLeft, ArrowRight, ArrowLeftRight, ArrowUp, Bell, BookOpen, CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, ClipboardList, Clock3, Cloud, CloudUpload, ExternalLink, FileText, Flame, Globe, Info, KeyRound, Landmark, Lock, MapPin, MessageSquare, Palette, Phone, Plus, RotateCcw, Settings as SettingsIcon, Share2, ShieldCheck, Trash2, Users, X } from 'lucide-react'
...
const colorNames: Record<string, string> = { '9x': 'My Group (9x day)', '6x': 'Partner Group (6x night)', MSOT: 'MSOT (PCOT)', RSOT: 'RSOT', 'Other OT': 'Other OT', Vacation: 'My Group on Vacation', Medical: 'Medical Leave', 'Crew On': 'Crew On', 'Crew Off': 'Crew Off', 'MX On': 'MX On (working for someone)', 'MX Off': 'MX Off (someone working for me)', 'Late Run': 'Late Run', 'Light Duty': 'Light Duty', 'Custom Event': 'Custom Event' }
function SettingsSection({ title, subtitle, badge, tone = 'gold', open = false, children }: { title: string; subtitle: string; badge?: string; tone?: 'green' | 'gold' | 'red' | 'plain'; open?: boolean; children: ReactNode }) {
  const number = title.slice(0, 2)
  return <details className="simx-section" open={open || undefined}><summary><span className="simx-section-text"><span className="simx-section-title"><b>{number} </b>{title.slice(2)}</span><small>{subtitle}</small></span>{badge && <span className={`simx-badge is-${tone}`}>{badge}</span>}<ChevronDown size={16} className="simx-chevron" /></summary><div className="simx-body">{children}</div></details>
}
function SettingsRow({ title, subtitle, meta, metaTone, lead, icon, danger = false, onClick }: { title: string; subtitle?: string; meta?: string; metaTone?: 'green' | 'red'; lead?: ReactNode; icon?: ReactNode; danger?: boolean; onClick: () => void }) {
  return <button className={`simx-row ${danger ? 'is-danger' : ''}`} onClick={onClick}>{lead && <span className="simx-row-lead">{lead}</span>}<span className="simx-row-text"><strong>{title}</strong>{subtitle && <small>{subtitle}</small>}</span>{meta && <span className={`simx-row-meta ${metaTone ? `is-${metaTone}` : ''}`}>{meta}</span>}{icon}</button>
}
...
export function AppSimulator() {
  ...
  function settingsScreen() {
    if (subpage === 'Edit Colors') return <>
      <SettingsRow title="Color Presets" subtitle="Restore the original app palette" lead={<RotateCcw size={16} />} icon={<ChevronRight size={15} />} onClick={() => { update({ colors: { ...defaultColors } }); setToast('Original app colors restored.') }} />
      <p className="sim-muted">Tour colors apply everywhere in the demo. Labels stay visible in every theme.</p>
      {Object.entries(data.colors).map(([name, value]) => <label className="simx-color-card" key={name}><input type="color" aria-label={`${name} color`} value={value} onChange={e => update({ colors: { ...data.colors, [name]: e.target.value } })} /><span><strong>{colorNames[name] ?? name}</strong><code>{value.toUpperCase()}</code></span><ChevronRight size={15} /></label>)}
    </>
    if (subpage === 'Operational Tools') return <>
      <SettingsRow title="Door Codes" subtitle="House & quarters access codes" lead={<KeyRound size={16} />} icon={<ChevronRight size={15} />} onClick={() => showPage('Door Codes')} />
      <SettingsRow title="Navigate to FDNY Unit" subtitle="Directions to your assignment" lead={<MapPin size={16} />} icon={<ChevronRight size={15} />} onClick={() => showPage('Navigate to FDNY Unit')} />
      <SettingsRow title="FDNY Links" subtitle="Department services & resources" lead={<Globe size={16} />} icon={<ChevronRight size={15} />} onClick={() => showPage('FDNY Links')} />
      <SettingsRow title="Telephone Numbers" subtitle="Desks, divisions & support lines" lead={<Phone size={16} />} icon={<ChevronRight size={15} />} onClick={() => showPage('Telephone Numbers')} />
    </>
    if (subpage === 'FDNY Links') return <>
      <SettingsRow title="FDNY Portal" subtitle="Department hub for members" lead={<Landmark size={16} />} icon={<ExternalLink size={15} />} onClick={() => info('FDNY Portal', 'This is a local resource preview. External services are disabled in the simulator.')} />
      <SettingsRow title="Divisions & Units" subtitle="Division and battalion listings" lead={<Globe size={16} />} icon={<ExternalLink size={15} />} onClick={() => info('Divisions & Units', 'This is a local resource preview. External services are disabled in the simulator.')} />
      <SettingsRow title="Member Resources" subtitle="Forms, contracts & benefits" lead={<BookOpen size={16} />} icon={<ExternalLink size={15} />} onClick={() => info('Member Resources', 'This is a local resource preview. External services are disabled in the simulator.')} />
    </>
    if (subpage === 'Telephone Numbers') return <>
      <p className="sim-muted">Fictional demo contacts. Calling is disabled in the simulator.</p>
      {[['Demo house desk', '(555) 010-0100'], ['Demo division desk', '(555) 010-0101'], ['Demo support', '(555) 010-0102']].map(([name, number]) => <SettingsRow key={name} title={name} subtitle="24-hour demo line" meta={number} lead={<Phone size={16} />} onClick={() => info(name, 'Demo number only. Calling is disabled.')} />)}
    </>
    ...
    return <>
      <SettingsSection title="01 IDENTITY & CREW" subtitle="Your identity, group & mutual partner" badge={data.partner ? 'CONNECTED' : 'OFFLINE'} tone={data.partner ? 'green' : 'red'}>
        <SettingsRow title="My Group & MX Partner" subtitle={data.partner ? 'Johnny Staylow · Joe Floorbelow · Group 10' : 'No mutual partner. Reset the demo to restore Joe.'} meta="10 / 10" lead={<Users size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('My Group & MX Partner', `Johnny Staylow · Group 10. ${data.partner ? 'Joe Floorbelow · Group 10.' : 'No mutual partner. Reset the demo to restore Joe.'}`)} />
        <SettingsRow title="My Crew Profile" subtitle="Rank, contact & unit details" lead={<Users size={15} />} icon={<ChevronRight size={15} />} onClick={() => showPage('My Crew Profile')} />
        <p className="sim-kicker">RANK</p>
        <Segments values={['Firefighter', 'Officer']} value={data.rank} onChange={rank => update({ rank })} label="Demo rank" />
      </SettingsSection>
      <SettingsSection title="02 OPERATIONAL TOOLS" subtitle="Door codes, navigation, links & numbers">
        <SettingsRow title="Open operational tools" subtitle="Codes, directions, links & phone numbers" lead={<Globe size={15} />} icon={<ChevronRight size={15} />} onClick={() => showPage('Operational Tools')} />
      </SettingsSection>
      <SettingsSection title="03 APPEARANCE" subtitle="Tour colors & presets" badge="CUSTOM" tone="gold">
        <SettingsRow title="Edit Colors" subtitle="Change tour colors for the calendar" lead={<Palette size={15} />} icon={<ChevronRight size={15} />} onClick={() => showPage('Edit Colors')} />
      </SettingsSection>
      <SettingsSection title="04 DATA & MAINTENANCE" subtitle="Backup, export & restore" badge={data.backedUp ? 'DEMO SNAPSHOT' : 'NOT BACKED UP'} tone={data.backedUp ? 'green' : 'red'}>
        <SettingsRow title="Cloud Auto Backup" subtitle="Automatic demo backup preview" meta={data.backedUp ? 'CURRENT' : 'NOT BACKED UP'} metaTone={data.backedUp ? 'green' : 'red'} lead={<CloudUpload size={15} />} onClick={() => { update({ backedUp: true }); setToast('Demo backup preview complete. No data left this browser.') }} />
        <SettingsRow title="Share Calendar" subtitle="Send a read-only demo copy" lead={<Share2 size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('Share Calendar · demo', 'Sharing is disabled. Nothing leaves this browser.')} />
        <SettingsRow title="Export Data · for New Phone" subtitle="Copy demo records as a file" lead={<ArrowUp size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('Export Data · demo', 'This preview does not export records. Reset demo restores the fictional sample.')} />
        <SettingsRow title="Import / Restore Data" subtitle="Restore from a demo file" lead={<Cloud size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('Import / Restore · demo', 'This preview does not import records. Reset demo restores the fictional sample.')} />
        <Accordion title="ADVANCED"><div>{row('Reset demo data', () => setModal({ kind: 'reset' }), 'Restore the original fictional sample')}</div></Accordion>
      </SettingsSection>
      <SettingsSection title="05 ABOUT & SUPPORT" subtitle="Version, legal & account" badge="v4.9.5" tone="plain">
        <SettingsRow title="App Version" subtitle="FDNY Mutual Tracker · interactive demo" meta="4.9.5" lead={<Info size={15} />} onClick={() => info('App version', 'Version 4.9.5 · Interactive demo. Not affiliated with the FDNY or City of New York.')} />
        <SettingsRow title="Terms of Use" subtitle="Demo license & limitations" lead={<FileText size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('Terms of Use', 'The full app’s terms apply on the App Store. This simulator is an independent demo with fictional data.')} />
        <SettingsRow title="Privacy Policy" subtitle="What the demo stores" lead={<Lock size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('Privacy Policy', 'Demo data stays in this browser’s local storage. No accounts, analytics, or personal data.')} />
        <SettingsRow title="View subscription preview" subtitle="Annual plan & 7-day trial" lead={<Flame size={15} />} icon={<ChevronRight size={15} />} onClick={() => setModal({ kind: 'paywall' })} />
        <SettingsRow title="Delete Account" subtitle="Disabled in this demo" danger lead={<Trash2 size={15} />} onClick={() => info('Delete Account · demo', 'Account actions are disabled. The demo has no account; reset the demo instead.')} />
        <p className="simx-disclaimer">FDNY Mutual Tracker is an independent app and is not affiliated with, endorsed by, or officially connected to the Fire Department of New York (FDNY).</p>
      </SettingsSection>
      <SettingsSection title="06 SEND FEEDBACK & SHARE" subtitle="Help improve the app">
        <SettingsRow title="Send Feedback" subtitle="Feature ideas & bug reports" lead={<MessageSquare size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('Feedback · demo', 'No message will be sent from this simulator. Use the website’s Send feedback button outside the phone if you want to contact the developer.')} />
        <SettingsRow title="Share the App" subtitle="Invite crew to the demo" lead={<Share2 size={15} />} icon={<ChevronRight size={15} />} onClick={() => info('Share the demo', 'Visitors can explore this same fictional app at /app-simulator. Browser-local changes are never shared.')} />
      </SettingsSection>
  ...
  return <section className={`app-simulator ${fullscreen ? 'sim-fullscreen' : ''}`}>
    ...
    <div className="container sim-workspace"><aside className="sim-side sim-side-left"><span className="sim-side-number">10–84</span><h2>This is a close representation of the app. It’s not an exact replica!</h2><p>You’re Johnny Staylow for this tour. Your partner is Joe Floorbelow. Everything here is fictional.</p><div className="sim-tour-guide"><button onClick={() => { navigate('Calendar'); setView('WEEK') }}><CalendarDays size={18} /><span><strong>Check your tour</strong><small>Pick a day. Add a sample entry.</small></span><ArrowRight size={15} /></button><button onClick={() => { navigate('Tracker'); setTracker('MUTUALS') }}><ArrowLeftRight size={18} /><span><strong>Keep mutuals straight</strong><small>See who owes the next tour.</small></span><ArrowRight size={15} /></button><button onClick={() => { navigate('Tracker'); setTracker('STATS') }}><Clock3 size={18} /><span><strong>Account for every hour</strong><small>Try the OT report comparison.</small></span><ArrowRight size={15} /></button></div><p className="sim-side-foot"><ShieldCheck size={17} /> No sign-in. No real records.</p></aside>
    <div ref={deviceShell} className="sim-device-shell" style={{ height: deviceScale < 1 && deviceHeight ? deviceHeight * deviceScale : undefined }}><div className={`sim-device ${tab === 'Calendar' && view === 'MONTH' ? 'sim-device-month' : ''}`} style={{ transform: deviceScale < 1 ? `scale(${deviceScale})` : undefined }}><div className="sim-device-status" aria-hidden="true"><span>9:41</span><i /><span className="sim-status-icons"><span className="sim-status-signal">{[0, 1, 2, 3].map(bar => <span key={bar} />)}</span><span className="sim-status-battery" /></span></div><header className={`sim-app-header ${tab === 'Settings' ? 'sim-app-settings' : ''} ${tab === 'Settings' && subpage ? 'sim-app-subpage' : ''}`}>{tab === 'Settings' && !subpage && <p className="sim-kicker">APP CONFIGURATION</p>}<div>{subpage && <button className={`sim-icon-button ${tab === 'Settings' ? 'sim-back-danger' : ''}`} aria-label="Back to section" onClick={() => showPage('')}><ArrowLeft size={18} /></button>}<h2 ref={heading} tabIndex={-1}>{subpage || tab}</h2><button className="sim-icon-button" aria-label="Demo recent activity" onClick={() => info('Recent activity', `${data.partner ? `Joe Floorbelow’s sample request is ${data.status.toLowerCase()}. ` : 'No active mutual requests. '}CFR-D training is due September 17. All activity is fictional.`)}><Bell size={19} /><span className="sim-notification-dot" /></button></div><p>{tab === 'Settings' && !subpage && <i className="sim-status-live" aria-hidden="true" />}L99 · GROUP 10 · DEMO</p></header>
