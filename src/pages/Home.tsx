...
import { ArrowDown, ArrowRight, ArrowUpRight, ArrowLeftRight, BookOpen, CalendarDays, Check, MapPin, MessageSquare, ShieldCheck, Users } from 'lucide-react'
...
export function Home({ onFeedback }: { onFeedback: () => void }) {
  return <>
    <section className="hero-section"><div className="container hero-grid">
      <div className="hero-copy"><p className="eyebrow hero-eyebrow"><span className="status-dot" /> V4.9.5 IS 10-84 <span className="eyebrow-divider" /> READY FOR YOUR NEXT TOUR</p><h1>BUILT BY AN FDNY<br />FIREFIGHTER.<br /><span className="brass-text">MADE FOR OUR JOB.</span><br /><span className="red-text">EVERYTHING WE NEED.</span></h1><p className="hero-description">Your tours. Your mutuals. Your overtime.<br />One app that knows the job.</p><div className="hero-tags"><span><CalendarDays size={15} /> Stay on schedule</span><span><ArrowLeftRight size={15} /> Keep mutuals straight</span></div><div className="home-simulator-cta"><Link className="button-red" to="/app-simulator">Try the app simulator <ArrowRight size={16} /></Link></div><a className="explore-link" href="#field-guide">Get more from your tracker <ArrowDown size={16} /></a></div>
      <PhoneCarousel />
    </div><div className="container hero-trust"><span><ShieldCheck size={15} /> Built by an active FDNY firefighter</span><span className="trust-separator" /><span><Users size={15} /> Made for the way we work</span></div></section>
