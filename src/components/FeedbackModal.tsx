...
export function FeedbackModal({ onClose, context }: { onClose: () => void; context: string }) {
  ...
  return <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose() }}><div className="feedback-modal" role="dialog" aria-modal="true" aria-labelledby="feedback-title" ref={ref}>
    ...
    {status === 'success' ? <div className="feedback-success"><div className="success-icon"><Check size={34} /></div><p className="eyebrow">RECEIVED. THANK YOU.</p><h2 id="feedback-title">YOU MAKE THIS<br />APP BETTER.</h2><p>Your feedback has been sent. I read and reply to every email, usually within 24 hours.</p><button className="button-red" onClick={onClose}>Back to the app <ArrowRight size={17} /></button></div> : <>
