...
try {
  ...
  const emptyMonthDay = demo.getByRole('button', { name: 'Sep 1, 2026', exact: true })
  await emptyMonthDay.dispatchEvent('pointerdown', { pointerId: 1, pointerType: 'touch', clientX: 20, clientY: 20 })
  await demo.waitForFunction(() => document.querySelector('.sim-device [aria-label="Calendar view"] button[aria-pressed="true"]')?.textContent === 'WEEK')
  check('Long-pressing an empty month day switches to Week view', await demo.getByRole('button', { name: 'WEEK', exact: true }).getAttribute('aria-pressed') === 'true' && await demo.getByRole('dialog').count() === 0)
  const monthViewButton = demo.getByRole('button', { name: 'MONTH', exact: true })
  await monthViewButton.focus()
  await demo.keyboard.press('Enter')
  await demo.getByRole('button', { name: 'Sep 1, 2026', exact: true }).press('Enter')
  check('Keyboard activation opens a month day after a long press without pointerup', await demo.getByRole('dialog').isVisible())
  await demo.keyboard.press('Escape')
  await demo.evaluate(() => window.dispatchEvent(new PointerEvent('pointerup', { pointerId: 1, pointerType: 'touch', clientX: 20, clientY: 20 })))
  // Mutual entries use the same .sim-reference-day button and therefore the same long-press destination.
  await demo.reload()
  await demo.locator('.sim-device').waitFor()
  await demo.getByRole('button', { name: 'MONTH', exact: true }).click()
  await demo.getByRole('button', { name: 'Sep 1, 2026', exact: true }).press('Enter')
